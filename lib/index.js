...
const parse = path => dotenv.config({ path }).parsed;

const sync = (envPath, envExamplePath) => {
  const syncedEnv = keepEnvsInSync(parse(envPath), parse(envExamplePath));
...
const dotenvSync = (filename = DEFAULT_EXAMPLE_ENV_FILENAME) => {
  ...
  if (!fileExists(EXAMPLE_ENV_PATH)) {
    writeToExampleEnv(DEFAULT_EXAMPLE_ENV_FILENAME, emptyObjProps(parse(ENV_PATH)));
